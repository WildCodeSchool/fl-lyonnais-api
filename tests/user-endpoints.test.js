...
// Test Cas où tout se passe bien, les 4 champs de la table user sont remplis et la case 
  describe('POST /registration', () => {
    describe('when a valid payload is sent', () => {
      ...
      it('returns the id of the created freelancer', async () => {
        expect(res.body.data).toHaveProperty('id');
      });
    });

    describe('when a freelancer with the same email already exists in DB', () => {
      let res;
      beforeAll(async () => {
        freelancer.create({
          first_name: 'John',
        ...
        res = await request(app).post('/freelancers').send({
          first_name: 'Jane',
          last_name: 'Doe',
          email: 'john.doe@gmail.com'
        });
